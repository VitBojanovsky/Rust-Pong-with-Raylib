...
fn main() {
    ...
    while !rl.window_should_close() {
        ...
        d.draw_text(&format!("Score: {}", score2), width - 100, 20, 20, Color::BLACK);
        ...
        if ballx < (borderx_raketa1 as f64) + 7.0 && bally > (bordery_raketa1 as f64) && bally < (bordery_raketa1 as f64) + 50.0 {
            ball_speed_x *= -1.0;
            score1 += 1;
        }
        if ballx > (borderx_raketa2 as f64) - 7.0 && bally > (bordery_raketa2 as f64) - 50.0 && bally < (bordery_raketa2 as f64) {
            ball_speed_x *= -1.0;
            score2 += 1;
        }
        if ballx - 7.0 < 29.0 { 
        ...
        //win condition
        if score1 >= 20 {
            d.draw_text("Player 1 Wins!", width/2 - 100, height/2 - 20, 40, Color::RED);
            sleep(Duration::from_millis(10000));
            break;
        }
        if score2 >= 20 {
            d.draw_text("Player 2 Wins!", width/2 - 100, height/2 - 20, 40, Color::BLUE);
            sleep(Duration::from_millis(10000));
            break;
        }  
